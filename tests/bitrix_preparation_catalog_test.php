...
$db->begin();$db->execute("INSERT INTO b_iblock_element (ID,IBLOCK_ID,ACTIVE) VALUES (2002,15,'N')");$db->execute("INSERT INTO b_catalog_product (ID,TYPE) VALUES (2002,4)");$db->execute('INSERT INTO b_iblock_element_prop_s15 (IBLOCK_ELEMENT_ID,PROPERTY_279) VALUES (2002,1001)');$ownedBindings=$bindings;$ownedBindings['variant']['parent_price_hash']=$hash;reject(fn()=>$capture($ownedBindings),'Foreign SKU invalidates parent authority before write');$ownedBindings['second']=['offer_id'=>'2002','scope_id'=>'site:s1','parent_price_hash'=>str_repeat('0',64)];reject(fn()=>$capture($ownedBindings),'Divergent per-variant parent provenance rejected');$ownedBindings['second']['parent_price_hash']=$hash;check($capture($ownedBindings)['derivedParentOwned'],'Partial update must retain coherent authority across all bindings');$db->rollback();
$db->begin();$empty();$db->execute('DELETE FROM b_iblock_element WHERE ID=2001');$db->execute('DELETE FROM b_catalog_product WHERE ID=2001');
$deleted=$bindings;$deleted['variant']['parent_price_hash']=str_repeat('a',64);
$gone=$capture($deleted);check($gone['missingBindings']===['variant'=>2001]&&$gone['derivedParentOwned'],'Native fully deleted SKU accepted with intact empty parent guards');
$db->execute('INSERT INTO b_catalog_price (ID,PRODUCT_ID,CATALOG_GROUP_ID,PRICE,CURRENCY) VALUES (99,2001,1,10,\'RUB\')');reject(fn()=>$capture($deleted),'Orphan price prevents recreation');$db->rollback();
$db->begin();$mix=$bindings;$mix['missing']=['offer_id'=>'2999','scope_id'=>'site:s1'];check($capture($mix)['missingBindings']===['missing'=>2999],'Unselected missing binding excluded from required snapshot');$db->execute("UPDATE b_iblock_element_prop_s15 SET PROPERTY_279='9999'");reject(fn()=>$capture($mix),'Mixed missing and moved retains moved conflict');$db->rollback();
$db->execute('CREATE TABLE b_pw_calc_preparation_write (id TEXT,preparation_id TEXT,scope_id TEXT,receipt_json TEXT,receipt_hash TEXT)');
$db->begin();$receipt=json_encode(['offerIds'=>['variant'=>2001],'variants'=>['variant'=>['state'=>$first['states'][2001]['state']]]]);$db->execute('INSERT INTO b_pw_calc_preparation_write VALUES (?,?,?,?,?)',['receipt','prep','site:s1',$receipt,hash('sha256',$receipt)]);
$partial=['variant'=>['offer_id'=>'2001','scope_id'=>'site:s1','parent_price_hash'=>str_repeat('a',64),'receipt_id'=>'receipt','preparation_id'=>'prep'],'gone'=>['offer_id'=>'2999','scope_id'=>'site:s1','parent_price_hash'=>str_repeat('a',64)]];
check($capture($partial)['derivedParentOwned'],'Partial deletion projection requires surviving exact receipt state');
$db->execute('UPDATE b_catalog_product SET WIDTH=17 WHERE ID=2001');reject(fn()=>$capture($partial),'Matching projected min alone cannot adopt changed survivor');$db->rollback();
