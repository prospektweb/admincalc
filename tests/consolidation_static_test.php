...
$basketAssets = (string)file_get_contents($root . '/features/orders/lib/BasketAssets.php');
...
foreach ([
    ...
    'Prospektweb\\LayoutFiles\\BasketAssets',
] as $class) {
...
consolidation_assert(strpos($patcher, 'BackupManager::createBackupBeforeInstall') === false, 'unsafe basket template patching must stay disabled');
consolidation_assert(strpos($manager, "'OnEpilog', '\\\\Prospektweb\\\\LayoutFiles\\\\BasketAssets'") !== false, 'runtime basket event missing');
consolidation_assert(strpos($basketAssets, "'/basket/'") !== false, 'basket runtime page guard missing');
consolidation_assert(strpos($version, "'VERSION' => '2.0.1'") !== false, 'consolidated version missing');
