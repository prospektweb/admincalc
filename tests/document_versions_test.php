...
$hiddenBefore = $cmd('loadVersion', ['versionId' => $branch]); $hiddenRegistry = $registry(); $hiddenHistory = $repo->history('sheet');
$rejects(fn() => $cmd('saveVersion', ['versionId' => $branch, 'expectedRevision' => 3, 'documentJson' => $body('Alternative')]), 409);
$rejects(fn() => $cmd('saveVersion', ['versionId' => $branch, 'expectedRevision' => 3, 'documentJson' => $body('Hidden overwrite')]), 409);
$rejects(fn() => $cmd('saveVersionConnection', ['versionId' => $branch, 'expectedRevision' => 3, 'connectionJson' => $connection]), 409);
$rejects(fn() => $cmd('restoreVersionRevision', ['versionId' => $branch, 'expectedRevision' => 3, 'revision' => 2]), 409);
$check($cmd('loadVersion', ['versionId' => $branch]) === $hiddenBefore && $registry() === $hiddenRegistry && $repo->history('sheet') === $hiddenHistory, 'Archived writes cannot append revisions, change bindings or move any pointer');
$check($cmd('previewVersion', ['versionId' => $branch, 'revision' => 3])['name'] === 'Alternative', 'Archived content remains readable and testable');
...
$change('archiveVersion', $primary, ['archived' => true]);
$hiddenPrimary = $repo->load('sheet'); $hiddenPrimaryRegistry = $registry();
$rejects(fn() => $repo->save('sheet', 4, $body('Hidden primary overwrite')), 409);
$rejects(fn() => $cmd('saveVersion', ['versionId' => $primary, 'expectedRevision' => 4, 'documentJson' => $body('Hidden named primary overwrite')]), 409);
$check($repo->load('sheet') === $hiddenPrimary && $registry() === $hiddenPrimaryRegistry && $repo->sitePublication('sheet') === $before, 'Primary compatibility save cannot bypass the archived version guard');
$change('archiveVersion', $primary, ['archived' => false]);
