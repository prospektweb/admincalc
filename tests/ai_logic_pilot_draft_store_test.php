...
try {
    $empty = $store->load(['presetId' => 12740, 'versionKey' => 'v_test', 'baseCompileHash' => $baseCompileHash]);
    ...
    if (($saved['revision'] ?? 0) !== 1) throw new RuntimeException('First revision was not saved.');
    $loaded = $store->load(['presetId' => 12740, 'versionKey' => 'v_test', 'baseCompileHash' => $baseCompileHash]);
    ...
    $changedSnapshot = $store->load(['presetId' => 12740, 'versionKey' => 'v_test', 'baseCompileHash' => str_repeat('b', 64)]);
    ...
    try {
        $store->save(['presetId' => 12741, 'versionKey' => 'v_test', 'baseCompileHash' => $baseCompileHash, 'draft' => $draft, 'decisions' => [], 'clientRevision' => 2]);
