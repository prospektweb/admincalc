...
$qaList=fn()=>$service->command($qaCommand('list',['productKey'=>'43']));
$current=array_values(array_filter($ql['items'],fn($r)=>$r['active']))[0];
$remove=['productKey'=>'43','expectedPreparationRevision'=>$ql['revision'],'resultIds'=>[$current['id']],'all'=>false];
$staleRestore=$service->command($qaCommand('preview',$qf));
$reject(fn()=>$service->command($qaCommand('remove',array_replace($remove,['all'=>true]))));
$reject(fn()=>$service->command($qaCommand('remove',array_replace($remove,['expectedPreparationRevision'=>0]))));
$beforeArchive=$db->rows('SELECT * FROM b_pw_calc_preparation_result ORDER BY id');
$db->execute("CREATE TRIGGER fail_archive BEFORE INSERT ON b_pw_calc_preparation_history BEGIN SELECT RAISE(ABORT, 'archive rollback'); END");
try{$service->command($qaCommand('remove',$remove));throw new RuntimeException('Expected SQL failure');}catch(PDOException $e){}
$ok($beforeArchive===$db->rows('SELECT * FROM b_pw_calc_preparation_result ORDER BY id'),'archive rollback atomic');$db->execute('DROP TRIGGER fail_archive');
$deleted=$service->command($qaCommand('remove',$remove));
$ok(count($deleted['items'])===1&&!$deleted['items'][0]['active'],'archive current does not promote history');
$reject(fn()=>$service->command($qaCommand('remove',$remove)));
$reject(fn()=>$service->command($qaCommand('load',['productKey'=>'43','resultId'=>$current['id']])),404);
$reject(fn()=>$service->command($qaCommand('transfer',$qf+['fingerprint'=>$staleRestore['fingerprint'],'choices'=>(object)[]])));
$restore=$service->command($qaCommand('preview',$qf));$ok($restore['counts']['new']===1,'explicit reimport offers archived snapshot');
$service->command($qaCommand('transfer',$qf+['fingerprint'=>$restore['fingerprint'],'choices'=>(object)[]]));
$ql=$qaList();$ok(count($ql['items'])===2&&array_values(array_filter($ql['items'],fn($r)=>$r['active']))[0]['id']===$current['id'],'restore same ID, no duplicate');
$allIds=array_column($ql['items'],'id');
$service->command($qaCommand('remove',['productKey'=>'43','expectedPreparationRevision'=>$ql['revision'],'resultIds'=>$allIds,'all'=>true]));
$ok(!$qaList()['items'],'clear includes history');
$restore=$service->command($qaCommand('preview',$qf));
$ok($restore['counts']['conflicts']===2,'clear followed by reimport requires winner among archived alternatives');
$service->command($qaCommand('transfer',$qf+['fingerprint'=>$restore['fingerprint'],'choices'=>[$restore['variants'][0]['key']=>$qa1]]));
$ql=$qaList();$ok(array_column($ql['items'],'id')===$allIds,'clear and restore retains exact IDs');
