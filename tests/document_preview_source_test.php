...
check($result['source'] === $source && $result['result']['values']->volume === 123 && $result['result']['values']->{'storefront:name'} === 'Test' && $result['result']['execution'] === $execution, 'Preview carries exact source and derived base storefront name');
check(!property_exists($values, 'storefront:name'), 'Preview does not mutate caller values');
