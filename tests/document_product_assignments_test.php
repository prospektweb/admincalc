...
$version=DocumentVersions::primaryId('sheet'); $items=[
    ['key'=>'42','name'=>'Existing cards','active'=>true,'adminUrl'=>'/bitrix/admin/iblock_element_edit.php?ID=42','siteUrl'=>'/catalog/existing/'],
    ['key'=>'43','name'=>'New sheets','active'=>false,'adminUrl'=>'/bitrix/admin/iblock_element_edit.php?ID=43','siteUrl'=>'https://configured.example/catalog/new/']
];
$hook=null; $reads=[];
$read=static function(array $queries,?array $ids) use (&$items,&$hook,&$reads): array {
    $reads[]=[$queries,$ids]; if($hook) { $callback=$hook;$hook=null;$callback(); }
    return array_values(array_filter($items,static fn($row)=>$ids!==null?in_array($row['key'],$ids,true):!array_filter($queries,static fn($query)=>!str_contains(mb_strtolower($row['name'].' '.$row['key']),mb_strtolower($query)))));
};
...
$catalog=$app->command($command('assignmentCatalog',['queries'=>['Existing','42']]));
$ok(count($catalog['items'])===1&&$catalog['items'][0]['adminUrl']==='/bitrix/admin/iblock_element_edit.php?ID=42'&&$catalog['items'][0]['siteUrl']==='/catalog/existing/'&&$catalog['source']['connectionHash']===hash('sha256',$before['connectionJson']),'Multi-term catalog search preserves dynamic links and exact source');
...
$fails(fn()=>(new DocumentProductAssignments($foreign,'bitrix:test','14',$read))->command($current('assignmentCatalog',['queries'=>['Existing']])),RuntimeException::class);
$fails(fn()=>(new DocumentProductAssignments($repo,'bitrix:foreign','14',$read))->command(['action'=>'assignmentCatalog','id'=>'sheet','versionId'=>$version,'expectedRevision'=>$saved['revision'],'queries'=>['Existing']]),DocumentConflict::class);
...
$catalog=$app->command(['action'=>'assignmentCatalog','id'=>'sheet','versionId'=>$version,'expectedRevision'=>$saved['revision'],'queries'=>['Existing']]);
...
$items=array_map(static fn($n)=>['key'=>(string)$n,'name'=>'P'.$n,'active'=>true,'adminUrl'=>'/bitrix/admin/iblock_element_edit.php?ID='.$n,'siteUrl'=>'/catalog/p'.$n.'/'],range(100,200));$reads=[];
...
$fails(fn()=>$app->command(['action'=>'assignmentCatalog','id'=>'sheet','versionId'=>$version,'expectedRevision'=>$saved['revision'],'queries'=>[str_repeat('x',101)]]),InvalidArgumentException::class);
$fails(fn()=>$app->command(['action'=>'assignmentCatalog','id'=>'sheet','versionId'=>$version,'expectedRevision'=>$saved['revision'],'queries'=>['same','SAME']]),InvalidArgumentException::class);
