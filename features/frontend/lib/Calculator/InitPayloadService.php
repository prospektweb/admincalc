...
use Prospektweb\Frontcalc\Config\ConfigManager;
...
class InitPayloadService
{
    ...
    /**
     * Подготовить INIT payload фронтового калькулятора для товара.
     *
     * @param int $productId ID товара каталога
     * @param array $virtualSelectedOffers Виртуальные selectedOffers, построенные из CALC_PROP_*
     * @param string $siteId ID сайта
     * @return array
     */
    public function prepareProductInitPayload(int $productId, array $virtualSelectedOffers, string $siteId): array
    {
        if ($productId <= 0) {
            throw new \InvalidArgumentException('ID товара должен быть больше нуля');
        }
        ...
        $preset = $presetId !== null ? $this->loadPreset($presetId) : null;
        ...
        return [
            ...
            'preset' => $preset ?: [],
            'product' => $product ?: ['id' => $productId, 'iblockId' => (new ConfigManager())->getProductIblockId()],
    ...
    public function prepareInitPayload(array $offerIds, string $siteId, bool $forceCreatePreset = false): array
    {
        ...
        if ($presetId === null || $forceCreatePreset) {
            throw new \RuntimeException('Для frontcalc требуется существующий CALC_PRESET у товара');
        }
    ...
    private function buildContext(string $siteId): array
    {
        ...
        return [
            ...
            'defaultExtraValue' => (float)Option::get(self::MODULE_ID, 'DEFAULT_EXTRA_VALUE', 0),
            'defaultExtraCurrency' => (string)Option::get(self::MODULE_ID, 'DEFAULT_EXTRA_CURRENCY', 'RUB'),
        ];
    }
