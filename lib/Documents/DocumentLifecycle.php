...
final class DocumentLifecycle
{
    ...
    public function delete(string $id, string $expected, string $name): array
    {
        return $this->transaction(function () use ($id, $expected, $name): array {
            ...
            foreach (['batch', 'snapshot_group', 'snapshot', 'product_binding', 'site_active', 'site_identity', 'version', 'catalog_write', 'site_publication', 'publication', 'revision'] as $table) {
    ...
    public function deleteVersionLocked(string $id, array $version): void
    {
        ...
        $this->db->execute('DELETE FROM b_pw_calc_snapshot_group WHERE document_id = ? AND version_id = ?', [$id, $versionId]);
        $this->db->execute('DELETE FROM b_pw_calc_snapshot WHERE document_id = ? AND version_id = ?', [$id, $versionId]);
        $this->db->execute('DELETE FROM b_pw_calc_batch WHERE document_id = ? AND version_id = ?', [$id, $versionId]);
