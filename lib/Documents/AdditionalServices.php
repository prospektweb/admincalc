        ...
        foreach ($document->form->fields ?? [] as $field) {
            if (property_exists($field, 'processWindow')) {
                ...
                if(property_exists($p,'storage')){
                    ...
                    $day=0;$amount=0;foreach($s->anchors as $index=>$a){self::shape($a,['days','value']);if(!is_int($a->days)||$a->days<1||$a->days>365||$a->days<=$day||!$money($a->value)||$a->value<$amount||($s->mode==='daily'&&$index<2&&$a->value!=0))self::fail('Некорректные отметки хранения');$day=$a->days;$amount=$a->value;}
